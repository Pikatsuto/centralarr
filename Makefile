...
$(VENV):
	python3 -m venv $(VENV)
	$(PIP) install -U pip
	cd $(BACKEND_DIR) && ../$(PIP) install -r requirements.txt && cd ..
...
dev: dev-install
	...
	cd $(FRONTEND_DIR) && npm run serve && cd .. &
	. $(VENV)/bin/activate && cd $(BACKEND_DIR) && uvicorn main:create_app --host 0.0.0.0 --port 5000 --reload --factory && cd ..